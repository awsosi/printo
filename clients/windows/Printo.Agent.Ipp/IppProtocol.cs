...
/// <summary>
/// IPP value and delimiter tags (RFC 8010 section 3.5). Only the tags the virtual printer
/// reads or writes are named; anything else round-trips as raw bytes, so an attribute we do
/// not understand is preserved rather than dropped.
/// </summary>
...
/// <summary>IPP operation ids the virtual printer answers (RFC 8011 4.4.15 / PWG 5100.x).</summary>
...
internal static class IppStatus
{
    ...
    public const ushort ClientErrorDocumentFormatNotSupported = 0x040A;
    public const ushort ServerErrorInternalError = 0x0500;
    public const ushort ServerErrorOperationNotSupported = 0x0501;
    public const ushort ServerErrorJobCanceled = 0x0508;
}
...
internal sealed class IppValue(IppTag tag, byte[] raw)
{
    ...
    /// <summary>Human-readable rendering, used by the capture log and the diagnostics page.</summary>
